#The original source of code is https://github.com/yyyyangyi/CNNs-for-Multi-Source-Remote-Sensing-Data-Fusion?tab=readme-ov-file
...
import random
import numpy as np
from utils.visualization import plot_layer_statistics
import torch
#from data.dataset_berlin import analyze_dataset_berlin
#from data.dataset_muufl import analyze_dataset_muufl
...
def main():
    ...
    #if Config.dataset == 'berlin':
    #    dataset_stats = analyze_dataset_berlin(Config.data_dir)
    #    print("\nDataset statistics loaded. Starting training for berlin dataset...\n")

    #if Config.dataset == 'muufl':
    #    dataset_stats = analyze_dataset_muufl(Config.data_dir)
    #    print("\nDataset statistics loaded. Starting training for muufl dataset...\n")
    ...
    for rep in range(num_replicates):
        ...
        if Config.result_out_dir is not None:
            ...
            #labels = range(Config.num_classes + 1)
            #conf_mats[rep] = confusion_matrix(y_true_all, y_pred_all, labels=labels, normalize=None)
    ...
    if Config.result_out_dir is not None:
        ...
        plot_layer_statistics(models, save_path=os.path.join(Config.result_out_dir, f'dgconv_stats_rep.png'))
